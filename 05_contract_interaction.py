...
def get_contract_code_tohex(target_address):
    return (web3.toHex(web3.eth.get_code(target_address)))
...
tx_hash = get_contract_instance(
    '0x876807312079af775c49c916856A2D65f904e612', '[{"inputs": [{"internalType": "string","name": "_key","type": "string"}],"stateMutability": "nonpayable","type": "constructor"},{"inputs": [],"name": "flag","outputs": [{"internalType": "string","name": "","type": "string"}],"stateMutability": "view","type": "function"}]').functions.test("test").transact()
...
def interact_with_contract_function(address, abi):
    ...
    txn_hash = target_contract.functions.test().transact()
